...
position = Position()
position.name = input('Введите имя сотрудника ')
position.surname = input('Введите фамилию сотрудника ')
position._income['wage'] = int(input('Введите оклад сотрудника '))
position._income['bonus'] = int(input('Введите премию сотрудника '))

print(f'Зарплата {position.get_full_name()} составит {position.get_total_income()} рублей')
